...
#Function for filtering out summer months
def is_summer(date):
  months = ['05','06','07','08','09','10']
  is_summer_month = []
  for month in months:
    is_summer_month.append(month in date[:2])
  return any(is_summer_month)
...
for i in range(len(data)):
  ...
  if(County_df.columns.values[2] == 'Ozone AQI Value'):
    ...
    summer_filter = Ozone_df['Date Oz'].apply(lambda x: is_summer(x))
    Ozone_df = Ozone_df[summer_filter]
    ...
    summer_filter = PM_df['Date PM'].apply(lambda x: is_summer(x))
    PM_df = PM_df[summer_filter]
...
final_df.to_csv('AQI_summer_avgs.csv')
